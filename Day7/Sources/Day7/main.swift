...
struct Vertex {
    ...
    let outgoing: [String]
    var incoming: [String]
    var weightChildren: Int
    
    init(name: String, weight: Int, outgoing: [String]) {
        self.name = name
        self.weight = weight
        self.outgoing = outgoing
        self.incoming = []
        self.weightChildren = 0
    }
}
...
for line in lines {
    let firstSpace = line.index(of: " ")!
    let name = String(line[line.startIndex..<firstSpace])
    let openBracket = line.index(of: "(")!
    let closeBracket = line.index(of: ")")!
    let weight = Int(line[line.index(after: openBracket)..<closeBracket])!
    var connections = [String]()
    let parts = line.components(separatedBy: " -> ")
    if parts.count == 2 {
        connections = parts[1].components(separatedBy: ", ")
    }
    vertices[name] = Vertex(name: name, weight: weight, outgoing: connections)
}

for (name, vertex) in vertices {
    for node in vertex.outgoing {
        vertices[node]?.incoming.append(name)
    }
}
...
func calculateWeights(node: String) {
    let vertex = vertices[node]!
    var weight = vertex.weight
    for childName in vertex.outgoing {
        calculateWeights(node: childName)
        weight += vertices[childName]!.weightChildren
    }
    
    vertices[node]!.weightChildren = weight
}

func determineWrongNode(node: String) -> String? {
    let vertex = vertices[node]!
    ...
    var subWeights = [Int]()
    for childName in vertex.outgoing {
        subWeights.append(vertices[childName]!.weightChildren)
        // If something is wrong with the node below this one, we must go deeper
        if let subNode = determineWrongNode(node: childName) {
            return subNode
        }
    }
    
    let maxSub = subWeights.max()!
    let minSub = subWeights.min()!
    let allMax = subWeights.filter( { $0 == maxSub }).count == subWeights.count
    let allMin = subWeights.filter( { $0 == minSub }).count == subWeights.count
    if !allMax || !allMin {
        print("Something wrong with subnodes of \(node)")
        return node
    }
...
calculateWeights(node: root)

let wrongNode = determineWrongNode(node: root)!

let vertex = vertices[wrongNode]!
var subWeights = [Int]()
for childName in vertex.outgoing {
    subWeights.append(vertices[childName]!.weightChildren)
}
...
let maxSub = subWeights.max()!
let minSub = subWeights.min()!
let maxValues = subWeights.filter( { $0 == maxSub })
let minValues = subWeights.filter( { $0 == minSub })
let allMax = subWeights.filter( { $0 == maxSub }).count
let allMin = subWeights.filter( { $0 == minSub }).count

let wrongValue = allMax >= (subWeights.count - 1) ? minSub : maxSub
let correctValue = wrongValue == maxSub ? minSub : maxSub
print(wrongValue, correctValue)
let diff = abs(wrongValue - correctValue)

for childName in vertex.outgoing {
    let v = vertices[childName]!
    if v.weightChildren == wrongValue {
