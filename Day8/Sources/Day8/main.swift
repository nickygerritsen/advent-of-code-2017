...
for line in input.split(separator: "\n") {
    ...
    let register = String(parts[0])
    if registers[register] == nil {
        registers[register] = 0
    }
    var val = Int(String(parts[2]))!
    switch parts[1] {
    case "inc":
        // Nothing
        break
    ...
    let registerToCheck = String(parts[4])
    if registers[registerToCheck] == nil {
        registers[registerToCheck] = 0
    }
    let registerCheckValue = Int(String(parts[6]))!
    let checked: Bool
    switch parts[5] {
    case ">":
        checked = registers[registerToCheck]! > registerCheckValue
    case "<":
        checked = registers[registerToCheck]! < registerCheckValue
    case "!=":
        checked = registers[registerToCheck]! != registerCheckValue
    ...
    if checked {
        registers[register] = registers[register]! + val
    }
    
    runningMax = max(runningMax, registers[register]!)
}

print(registers.max(by: { return $0.1 < $1.1 }))
print(runningMax)
