func isValidPassphrase(passphrase: String.SubSequence) -> Bool {
    let words = passphrase.split(separator: " ")
    var wordCounts = [String.SubSequence: Bool]()
    for word in words {
...
func isValidPassphraseWithAnagram(passphrase: String.SubSequence) -> Bool {
    let words = passphrase.split(separator: " ")
    var wordCounts = [String: Bool]()
    for word in words {
        let anagramed = String(word.sorted())
        if wordCounts[anagramed] != nil {
            return false
        }
        wordCounts[anagramed] = true
    }
...
for passphrase in input.split(separator: "\n") {
    if isValidPassphrase(passphrase: passphrase) {
        cnt1 += 1
    }
    if isValidPassphraseWithAnagram(passphrase: passphrase) {
        cnt2 += 1
    }
}
